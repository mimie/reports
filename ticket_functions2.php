<!--
  ...
  /*functions for daily_closedTickets main class
...
<?php
    ...
    */
   function getLinkedTickets($source_key,$activationDate){
        ...
        $sql_linkedClosedTickets = "SELECT lr.source_key,lr.target_key,t.tn,t.title,t.id FROM `link_relation` lr, ticket t \n"
         ...
         while($row = mysql_fetch_row($result_linkedTickets)){
          ...
          if($count_results > 0){
             ...
             $sql_createTime = mysql_query("SELECT create_time FROM ticket WHERE id = '$ticketId'") or die(mysql_error());
             $rowtime = mysql_fetch_row($sql_createTime);
             $createTime = $rowtime[0];

             $checkDate = checkDateWithin30Days($createTime,$activationDate);
  ...
  function getTicketAge($circuitActivationDate,$ticketCreateDate){
   ...
   function getCircuitMaturity($circuitActivationDate, $oldestTicketCreateDate){
      ...
      $maturity = intval($maturity);

      if($maturity == 0){

        $maturity = $interval->format('%a') + 1;
        return $maturity." Day old from first outage";
      }
  ...
  function getDailyClosedTickets(){
       ...
       $result_closedTickets = mysql_query("SELECT th.create_time, ticket_id,tn,title FROM ticket_history th, ticket t
                               WHERE th.create_time BETWEEN '$yesterday 00:00:00:00' AND '$yesterday 23:59:59:59'
  ...
  /**
   *getActivationDate
   *@param (xml_key) - the xml_key is from the xml_storage table
   *                 - it is also the last_version_id of the configitem table
   *@return (activation date) - the activation date of the circuit Id
   *                          - thea activation date is xml_content_key = [1]{'Version'}[1]{'ActivationDate'}[1]{'Content'}
   */
  ...
  /**
   *checkDateWithin30Days
   *@param (ticket Create Date/Time) - date/time when the ticket is created
   *@param (circuit Activation Date) - the activation date of the linked ticket circuit id
   *@return (boolean true or false) - return true if ticket create time is within 30 days from the activation date of the circuit id
   *                                - return false if ticket create time is not within 30 days from the activation date of the circuit id
   *                                  OR if ticket create time is earlier than circuit activation date
   */
  ...
  /**
   *generatePDF - generates pdf file of the report
   *@param (html) - html contents of the the generated php file output
   *
   */
  function generatePDF($html){
    ...
    return $fileName."_closedTickets.pdf";
 
  }

  function getDeploymentState($last_version_id){
    
    $sql_deploymentState = "SELECT cur_depl_state_id FROM configitem WHERE last_version_id = '$last_version_id'";
    $result_deploymentState = mysql_query($sql_deploymentState);
    $row = mysql_fetch_row($result_deploymentState);
    $deploymentState_id = $row[0];

    return $deploymentState_id == 147;
